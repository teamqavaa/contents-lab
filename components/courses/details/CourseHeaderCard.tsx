...
export default function CourseHeaderCard({ course }: CourseHeaderCardProps) {
  ...
  return (
    <div className="relative w-full aspect-[16/10] sm:aspect-video rounded-3xl overflow-hidden bg-neutral-900 border border-neutral-200 shadow-md font-mono group">
      ...
      {isPlayingVideo && course.promo_video_url ? (
        ...
        <>
          ...
          <button
            ...
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-white/90 text-neutral-900 flex items-center justify-center shadow-xl backdrop-blur-md hover:bg-white hover:scale-110 transition-all cursor-pointer"
      ...
      {!isPlayingVideo && (
        <div className="absolute inset-0 z-20 p-6 sm:p-10 flex flex-col justify-between pointer-events-none">
