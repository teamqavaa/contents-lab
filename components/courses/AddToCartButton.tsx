...
import { addToCartAction, getMyCart } from '@/actions/cart';
...
const SSO_LOGIN_URL =
  process.env.NEXT_PUBLIC_SSO_URL || 'http://localhost:3001';
    ...
    const checkCartStatus = async () => {
      try {
        ...
        if (cartData && cartData.items) {
          const itemExists = (cartData.items || []).some((item: any) => {
            const idToCheck =
              item.course_details?.id || item.course?.id || item.courseId || item.course_id || item.id;
